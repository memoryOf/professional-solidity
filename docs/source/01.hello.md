...
contract Demo {
    ...
    function test() public pure returns (bool a,bool b,bool c) {
        a = 1 wei == 1;
        b = 1 gwei == 1e9;
        c = 1 ether == 1e18; //等价于 c = 1 ether == 1 *  10**18;
    }
}
```
...
之后，它们就变成了普通·函数，只能按一般函数来去调用。同时 `receive` 和 `fallback` 需要注意 gas 消耗。
...
contract Payable {
    ...
    //调用deposit2并发送eth;transaction revert 
...
可以使用 payable deposit 存款，但是如果使用 calldata 转账，则会失败，报错 _In order to receive Ether transfer the contract should
...
直接使用方法是不行的，即调用test1会revert；但是可以通过 call 调用，因为 call 不检查，这也官方是不推荐使用 call 的原因。
