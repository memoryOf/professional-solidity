...
的目标来接收以太币。一个合约不能对这种以太币转移做出处理，因此也不能拒绝它们。这是 EVM 在设计时就决定好的，而且 Solidity
...
contract Demo {
    ...
    // 任何对合约 非空calldata 调用会执行回退函数(即使是调用函数附加以太).
    fallback() external payable {
        x = 2;
        y = msg.value;
        emit Log("fallback", msg.sender, msg.value, msg.data);
...
⚠️注意：`receive` 函数可能只有 2300 gas 可以使用（如，当使用 `send` 或`transfer` 时），

⚠️注意: 一个没有定义 `fallback` 函数或 `receive` 函数的合约，直接接收以太币（没有函数调用，使用 `send` 或 `transfer`）会抛出一个异常，
...
You're getting a warning about using selfdestruct!

这个警告的意思是，你正在使用一个被称为 selfdestruct 的函数，但它已经被弃用（deprecated）。
从 Cancun 硬分叉开始，执行此函数时，不再会彻底删除账户相关的代码和数据，而 **只是将 Ether 转移到指定受益人那里。**

这个变化是由 EIP-6780 引入的。即使你考虑了新的行为，也强烈建议不要在新部署的合约中使用 selfdestruct，因为未来可能会对其功能进行进一步限制。

简单来说，就是要避免用 selfdestruct 了！
...
为此我写了这个阅兵式里`同志们好`的场景合约，用于能力自检，相当于加强版的 Hello World。
...
```
  Status public step = Status.Step0;
  enum Status {
      Step0,
      Step1,
      Step2,
...
- receive 和 fallback 区别？
    - receive 只负责接收主币
    - fallback 调用没有的方法时候执行,因为可以设置 payable，可以接收网络主币。尽管 fallback 可以是 payable 的，但并不建议这么做，声明为
